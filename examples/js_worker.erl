...
-doc "Start a worker over a QuickJS build. `Opts` is `wasm_script_worker`'s.".
-spec start_link(file:filename_all(), map()) -> {ok, pid()} | {error, term()}.
start_link(EnginePath, Opts) ->
    wasm_script_worker:start_link(wasm_javascript_command, Opts#{path => EnginePath}).

-spec start_link(gen_server:server_name(), file:filename_all(), map()) ->
          {ok, pid()} | {error, term()}.
start_link(Name, EnginePath, Opts) ->
    wasm_script_worker:start_link(Name, wasm_javascript_command, Opts#{path => EnginePath}).
