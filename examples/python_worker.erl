...
-doc "Start a worker over a CPython build. `Opts` is `script_worker`'s.".
-spec start_link(file:filename_all(), map()) -> {ok, pid()} | {error, term()}.
start_link(EnginePath, Opts) ->
    script_worker:start_link(py_adapter, Opts#{path => EnginePath}).

-spec start_link(gen_server:server_name(), file:filename_all(), map()) ->
          {ok, pid()} | {error, term()}.
start_link(Name, EnginePath, Opts) ->
    script_worker:start_link(Name, py_adapter, Opts#{path => EnginePath}).
