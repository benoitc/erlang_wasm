...
Was `qjs_worker`, which is now the language-neutral worker kernel. This
module kept the name's original job: the smallest thing that runs a script and
returns what it printed. It is deliberately unchanged, so the tradeoffs below
still describe what it does, and it is the baseline the kernel is measured
against rather than a deprecated copy of it.
...
```erlang
{ok, W} = qjs_worker:start_link("test/fixtures/lang/qjs.wasm"),
{ok, ~"3\\n"} = qjs_worker:eval(W, ~"print(1 + 2);"),
{error, timeout} = qjs_worker:eval(W, ~"for(;;){}").
```
