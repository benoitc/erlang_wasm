-module(wasm_javascript).
-moduledoc """
Run JavaScript from an image of an already-started engine.

Same profile and the same tenant contract as `wasm_javascript_command` -- `main(context)`,
...
## Why this one can be snapshotted and `wasm_javascript_command` cannot
...
decode(#{outcome := exited, exit := Code} = R, _State) ->
    ...
    {error, wasm_worker_error:adapter(exit, ~"the engine exited", #{code => Code,
                                                              stdout => Out,
                                                              stderr => Err})}.
